...
\chapter{Geometric Localization of Waves on Thin Elastic Structures}
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/waves.pdf}
  \end{center}
  \caption{
...
%
\begin{equation}
  \partial_{t}^{2}\Psi(x,t) + \hat{\mathsf{H}}\Psi(x,t) = 0,
...
Also, $\hat{\mathsf{H}}$ is taken to be a Hermitian operator in the form of an $N\times N$ matrix, composed solely of spatial derivatives (i.e., powers of $\partial_{x}$) with time-independent coefficients.
...
%
\begin{equation}
  \hat{\mathsf{D}}\psi = 0,\quad \text{with}\enspace \hat{\mathsf{D}} = \hat{\mathsf{H}} - \omega^{2}\mathsf{I}_{N},
  \label{eq:ev_problem}
\end{equation}
%
where $\mathsf{I}_{N}$ is the $N\times N$ identity matrix.
If the coefficients of the spatial derivatives that appear in $\hat{\mathsf{D}}$ are constants, then the eigenmodes $\psi$ are plain waves.
...
With analogy to quantum mechanics, this allows us to recast the derivatives in $\hat{\mathsf{D}}$ in terms of the wave number/momentum operator $\hat{k} = -i\epsilon \partial_{x}$, with $\epsilon$ playing the role of Planck's constant.
Since we shall be considering $\hat{\mathsf{D}}$ in the coordinate representation, the position operator $\hat{x} = x$.
...
Converting each entry of the matrix operator $\hat{\mathsf{D}}$ into a Weyl symbol, we get the $N\times N$ dispersion matrix $\mathsf{D}$, which we express in various orders of $\epsilon$ as $\mathsf{D} = \mathsf{D}^{(0)} + \epsilon\mathsf{D}^{(1)} + \mathcal{O}(\epsilon^{2})$.
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/caustic.pdf}
  \end{center}
  \caption{%
...
We nondimensionalize the rod equations by setting the length units to $\ell = \sqrt{B/E} \sim h$ and the time units to $\sqrt{B\rho}/E$.
...
For the eigenvalue problem with $\hat{\mathsf{D}} = \hat{\mathsf{H}} - \omega^{2}\mathsf{I}_{2}$, this gives us
%
\begin{equation}
 \hat{\mathsf{D}} =
 \begin{pmatrix}
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/rod_profile.pdf}
  \end{center}
  \caption{%
...
In Appendix~\ref{app:extensional} we present an alternative approach, where we analyze the extensional limit of the rod equations and show that extensional waves form bound states.
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/rod_bound.pdf}
  \end{center}
  \caption{%
...
Two example bound states obtained by solving the rod equations numerically (see Appendix \ref{app:numerical} for details) are shown in Figs.~\ref{fig:rod_bound}(c) and~\ref{fig:rod_bound}(d).
...
% Incidentally, this cut-on frequency is the ring frequency of extensional waves, i.e., when the wavelength is equal to the circumference of a ring of radius $m^{-1}$.
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/rod_wkb.pdf}
  \end{center}
  \caption{
...
Now, note that the off-diagonal operators of the matrix $\hat{\mathsf{D}}$ in the rod equations, Eq.~\eqref{eq:rod}, are composed only of odd derivatives.
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/rod_extended.pdf}
  \end{center}
  \caption{%
...
%Again, the ray trajectories are invariant under reflection about the $x$ and $k$ axes because $\lambda_{-}$ possesses the same symmetry.
%At large frequencies, the rays appear straight, and they start to resemble plain waves.
%
...
On setting the length units to $\sqrt{\widetilde{B}/\widetilde{E}}$ and time units to $\sqrt{\widetilde{B}\varrho}/\widetilde{E}$, we arrive at the nondimensional form of these equations, which in matrix form reads
%
\begin{equation}
  \partial^{2}_{t}
  \begin{pmatrix}
...
\end{equation}
%
where the operator $\hat{\mathsf{H}}$ is defined by
%
\begin{equation}
  \hat{\mathsf{H}} =
  \begin{pmatrix}
...
\end{equation}
%
A set of equations analogous to the rod equations [see Eq.~\eqref{eq:rod}] can be obtained by suppressing the $y$ derivatives in the submatrix obtained by deleting the third row and column of the operator $\hat{\mathsf{H}}$ given above.

Translation invariance of $\hat{\mathsf{H}}$ along $y$ lets us look for time-harmonic solutions with the common factor $e^{i(ly - \omega t)}$, where $l$ is the transverse wave number in the $y$ direction and $\omega$ is the frequency of oscillation.
This makes the wave field depend only on the coordinate $x$, and makes the transverse wave number $l$ an additional parameter of the operator $\hat{\mathsf{H}}$.
But the operator $\hat{\mathsf{H}}$ now has complex coefficients, and the components of its eigenmodes are complex functions.
...
Finally, we find the dispersion matrix $\mathsf{D}^{(0)}$ and its first-order correction $\mathsf{D}^{(1)}$ to be
%
\begin{equation}
\mathsf{D}^{(0)} =
\begin{pmatrix}
...
\end{equation}
%
For later analysis, it is also useful to note down the determinant of $\mathsf{D}^{(0)}$, which is
%
\begin{equation}
  \begin{aligned}
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/shell_disp.pdf}
  \end{center}
  \caption{%
...
Also, to find the dispersion relations from Eq.~\eqref{eq:shell_det} we now need to use the general cubic formula~\cite{olver2010}, which results in unwieldy analytical expressions~\cite{mannattil2023}.
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/shell_gap.pdf}
  \end{center}
  \caption{%
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/shell_rays.pdf}
  \end{center}
  \caption{%
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/shell_modes_tanh.pdf}
  \end{center}
  \caption{
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/shell_wkb.pdf}
  \end{center}
  \caption{
...
Given the qualitative similarity between the bound states observed with such a profile and those seen in this paper, they will be discussed elsewhere~\cite{mannattil2023}.
...
Putting Eq.~\eqref{app:eq:tau} in Eq.~\eqref{eq:extra_phases}, we see that the rate of change of the first (geometric) phase $\gamma_{\text{G}}$ is
%
\begin{equation}
  \begin{aligned}
...
\begin{equation}
  ...
  \label{app:eq:gamma_NG}
  \end{aligned}
\end{equation}
%
In the last step above, we have used Eq.~\eqref{app:eq:tau} to simplify the first term on the RHS and have defined $\varphi_{jk} = \varphi_{j} - \varphi_{k}$.
...
From Eq.~\eqref{app:eq:gamma_NG} we see that even when the phases $\varphi_{j}$ are constants, $\gamma_{\text{NG}}$ could be nonzero.
...
%
\begin{equation}
  \partial_{t}^{2}\Psi(x, y, t) + \hat{\mathsf{H}}\Psi(x, y, t) = 0
  \quad\text{with}\quad
  \hat{\mathsf{H}} =
  \begin{pmatrix}
    \hat{Z} & \hat{A} & \hat{B}\\
  ...
  \end{pmatrix},
  \label{app:eq:wave_eq}
\end{equation}
%
Above, the entries of $\hat{\mathsf{H}}$ are linear differential operators comprised of powers of $\partial_{x}$ and $\partial_{y}$.
...
Additionally, since Eq.~\eqref{app:eq:wave_eq} represents an elastodynamic system, we assume that the coefficients of all the derivatives in $\hat{\mathsf{H}}$ are real so that $\Psi$ can be taken to be real as well.

Any invariance possessed by Eq.~\eqref{app:eq:wave_eq} must be shared by the (potential) energy density $\mathscr{J} = \frac{1}{2}\Psi\trans\hat{\mathsf{H}}\Psi$ used to derive it from Hamilton's principle.
...
Using the rules in Eq.~\eqref{eq:weylrules}, we therefore conclude that the lowest-order symbols of the off-diagonal operators  $\hat{A}$ and $\hat{B}$ must be purely complex (as $\hat{\mathsf{H}}$ did not have complex coefficients to begin with).
...
%
As $\hat{\mathsf{H}}$ is a Hermitian operator, the symbols of the diagonal entries are all real and $\mathcal{O}(\epsilon)$ corrections to these symbols must vanish.

For finding the eigenmodes, after Fourier transforming in time, we define $\hat{\mathsf{D}} = \hat{\mathsf{H}} - \omega^{2}\mathsf{I}_{3}$ and convert $\hat{\mathsf{D}}$ to its symbol form $\mathsf{D} = \mathsf{D}^{(0)} + \epsilon\mathsf{D}^{(1)} + \mathcal{O}(\epsilon^{2})$.
...
\begin{pmatrix}
  ...
  B^{(1)} & -iC^{(1)} & 0
\end{pmatrix}.
\label{app:eq:gen_disp_matrix}
\end{equation}

A polarization vector $\tau$ is defined up to an overall phase and normalization by $\mathsf{D}^{(0)}\tau = 0$.
Direct inspection reveals that for $\mathsf{D}^{(0)}$ defined in Eq.~\eqref{app:eq:gen_disp_matrix}, we can take $\tau$ to be of the form%
\footnote{To see this more explicitly, take $\tau = (r_{1}e^{i\phi_{1}},\, \tau_{2},\, \tau_{3})$ and solve for the components $\tau_{2}$ and $\tau_{3}$ from $\mathsf{D}^{(0)}\tau = 0$.
  Upon rephasing $\tau$ by $e^{-i\phi_{1}}$, we find that $\tau$ is of the general form in Eq.~\eqref{app:eq:tau3}.
}
%
\begin{equation}
  \tau =
...
When the matrix $\mathsf{D}^{(1)}$ is of the form in Eq.~\eqref{app:eq:gen_disp_matrix}, using the polarization vector $\tau$ in Eq.~\eqref{app:eq:tau3}, a straightforward computation shows that second term in the expression for $\dot{\gamma}_{\text{NG}}$, Eq.~\eqref{app:eq:gamma_NG}, vanishes.
...
From Eq.~\eqref{app:eq:gamma_NG} it then follows that $\dot{\gamma}_{\text{NG}}$ vanishes, and we can take $\gamma_{\text{NG}}$ to be zero as well.

The dispersion matrices for the thin shell we considered in Eq.~\eqref{eq:shell_disp_matrices} of the main text is of the form in Eq.~\eqref{app:eq:gen_disp_matrix}, and hence the phase $\gamma = \gamma_{\text{G}} + \gamma_{\text{NG}}$ is zero for the shell.
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/rod_poschl.pdf}
  \end{center}
  \caption{%
    ...
    The dashed black curves in (a) and (b) represent the solutions in Eq.~\eqref{app:eq:poschl}.
...
We now look for bound-state solutions of Eqs.~\eqref{app:eq:rod_ext1} and \eqref{app:eq:rod_ext2} satisfying $\zeta(\pm\infty) = u(\pm\infty) = 0$ with vanishing derivatives at $x = \pm\infty$.
...
Without this assumption, on integrating Eq.~\eqref{app:eq:rod_ext2} we see that $\phi(\pm\infty) = C$ (constant).
We then obtain Eq.~\eqref{app:eq:schrodinger} in terms of $\widetilde{\phi}(x) = \phi(x) - C$ with $\zeta = m\widetilde{\phi}$, so that we can work in terms of $\widetilde{\phi}$ alone, which is equivalent to setting $C = 0$.}
$\phi(\pm\infty) = 0$.
Equation~\eqref{app:eq:rod_ext2} then becomes a total derivative, which on integration yields
%
\begin{equation}
  m(x)\zeta(x) = \partial_{x}^{2}\phi(x) + \omega^{2}\phi(x).
\end{equation}
%
Putting the above equation in Eq.~\eqref{app:eq:rod_ext1} and ignoring the soft-mode solution with $\omega = 0$, we see that $\zeta(x) = m(x)\phi(x)$, from which we deduce that $\phi(x)$ satisfies a Schr\"{o}dinger-like equation with the potential $m^{2}(x)$, and given by
%
\begin{equation}
  -\partial_{x}^{2}\phi(x) + m^{2}(x)\phi(x) = \omega^{2}\phi(x).
  \label{app:eq:schrodinger}
\end{equation}
%
As is well known from elementary quantum mechanics~\cite{buell1995}, Eq.~\eqref{app:eq:schrodinger} always admits a bound-state solution provided that potential $m^{2}(x)$ has a minimum.

For the tanh-type curvature profile with $m = m_{1}(x) = b\tanh(\epsilon x)$, Eq.~\eqref{app:eq:schrodinger} becomes the time-independent Schr\"{o}dinger equation for a particle in a P\"{o}schl--Teller potential~\cite{poschl1933,rosen1932}, whose solutions $\phi(x)$ can be written in terms of the associated Legendre polynomials $P^{\mu}_{\nu}(\,\cdot\,)$~\cite{olver2010}.
On defining
%
\begin{equation}
...
\end{equation}
%
Figure~\ref{fig:poschl} presents a comparison of the bound states described by Eq.~\eqref{app:eq:poschl} and the numerical results for extensional bound states.
...
From Eq.~\eqref{app:eq:poschl_params} we also see that maximum number of bound states is $n_{\text{max}} \approx \nu = b/\epsilon + \mathcal{O}(b^{2}/\epsilon^{2})$, which agrees with the semiclassical prediction in Eq.~\eqref{eq:rod_maxb}.

We are currently unaware of an exact solution of Eq.~\eqref{app:eq:schrodinger} for the potential $\smash{m^{2}_{2}(x) = \left[b - (b - a)\sech(\epsilon x)\right]^{2}}$ corresponding to a sech-type curvature profile, even though exact solutions for similar potentials exist~\cite{lemieux1969,nieto1978,ishkhanyan2018}.
Although we have had success in approximating this potential as a modified P\"{o}schl--Teller potential~\cite{mannattil2023},
as the results are qualitatively similar, we do not report them here.
...
\end{equation}
%
and we fix the deformation parameter $\chi$ by demanding that the full widths at half maxima of both $m^{2}_{\chi}(x)$ and $m_{2}^{2}(x)$ be equal, which gives
%
\begin{equation}
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/rod_poschl_sech.pdf}
  \end{center}
  \caption{%
    ...
    Lighter curves in (a) and (b) depict higher-frequency states and the dashed black curves represent the solutions obtained analogously to those in Eq.~\eqref{app:eq:poschl} after setting $b^{2} \to b^{2} - a^{2}$ and $\epsilon \to \chi\epsilon$.
...
At this juncture, let us remark that Eq.~\eqref{app:eq:schrodinger} can also be derived by diagonalizing the full wave equation in Eq.~\eqref{eq:rod} using the asymptotic method outlined by Littlejohn and coworkers~\cite{littlejohn1991,littlejohn1991a,weigert1993}, without appealing to any physical arguments.
...
Next, we promote $k \to \hat{k} = -i\partial_{x}$, which ``quantizes'' the ray Hamiltonian $\lambda_{+}$ and we obtain Eq.~\eqref{app:eq:schrodinger} for an unknown wave function $\phi(x)$.
...
\footnote{The code we use for all our numerical calculations is publicly available~\cite{github}.}
...
% Exact dispersion for a curved shell
% -----------------------------------
%
% \subsection{Exact dispersion relations for a curved shell}
%
% \hl{Manu: remove this.}
% The exact dispersion relations for plain waves propagating on a singly curved shell can be obtained by solving the following cubic in $\omega^{2}$:
% %
% \begin{equation}
...
%     c &= \frac{1}{2}\left[q^{8} + m^{2}l^{4}\left(1 - \eta^{2}\right)\right],
%   \end{aligned}
% \end{equation}
% %
...
% \end{equation}
% %
% The ray trajectories depicted in Fig.~\ref{fig:shell_rays} can be obtained by treating $\omega^{2}$ in Eq.~\eqref{app:shell_cubic_sol} as the ray Hamiltonian.
% However, as we described in the main text, in practice, it is more convenient to use the determinant $\Lambda$ of the dispersion matrix [Eq.~\eqref{eq:shell_det}] with appropriate initial conditions to obtain the ray trajectories and for finding the frequencies of the quantized bound modes.
